...
$.fn.bookingFilterTime = ->
  @each ->
    $(@).on 'change', ->
      ...
      user = getCurrentUser()
      if user.role == 'chief' && ((dateFrom.isoWeekday() == 7 && dateFrom.hours() < 18 && dateFrom.hours() >= 8) || (dateTo.isoWeekday() == 7 && dateTo.hours() >= 8 && dateTo.hours() < 18))
      ...
      else if user.role == 'maker' && (dateFrom.isoWeekday() != 7 || dateTo.isoWeekday() != 7 || dateTo.hours() > 18 || dateFrom.hours() < 8 || (dateTo.hours() == 18 && dateTo.minutes() > 0))
      ...
      else if timesState.total < 2
        $(document).trigger('map:loading:change', [on, 'Booking must be at least 2 hours.'])

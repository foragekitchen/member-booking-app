  ...
  @each ->
    $(@).on 'change', ->
      ...
      if dateFrom.isBefore(moment(new Date()))
        $(document).trigger('map:loading:change', off, 'Booking cannot be in the past.')
      else if timesState.total < 4
        $(document).trigger('map:loading:change', off, 'Booking must be at least 4 hours.')
      else if timesState.total > 12
        $(document).trigger('map:loading:change', off, 'Booking cannot be more than 12 hours.')
      else

...
- Troubleshooting / gotchas:
  - Tests failing? Sometimes this can be due to the time of day you're running the tests, since the Kitchen's *timeslots are set to close from 2AM-8AM* (yes, this means they are a little bit brittle and can be refactored, but on the flipside should mirror real life conditions). Though the _stubbed_ tests (ones that don't make a live connection to the Nexudus server) also stub out the current date/time, the *_live_ tests rely on the actual current time.*

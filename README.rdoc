...
== Tests:
Because this App primarily is an API wrapper around Nexudus and makes a lot of calls to Nexudus's API, the test suite focuses mainly on *feature/integration tests* that fire up the Selenium browser (via Capybara - remember "js:true"!) and tests end-to-end interactions. The tests switch between using fake data and stubbing live data calls, vs making _actual live changes_ to the Nexudus database. This means you should be very very careful running the test suite on a live production environment; ideally there is always a *clone of the production environment that is used for testing* instead (a clone where no actual users have access).

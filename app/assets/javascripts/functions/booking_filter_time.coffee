...
$.fn.bookingFilterTime = ->
  @each ->
    $(@).on 'change', ->
      ...
      user = getCurrentUser()
      if !user.maker && ((dateFrom.isoWeekday() == 7 && dateFrom.hours() < 18 && dateFrom.hours() >= 8) || (dateTo.isoWeekday() == 7 && dateTo.hours() >= 8 && dateTo.hours() < 18))
      ...
      else if user.maker && (dateFrom.isoWeekday() != 7 || dateTo.hours() > 18 || dateFrom.hours() < 8 || (dateTo.hours() == 18 && dateTo.minutes() > 0))
